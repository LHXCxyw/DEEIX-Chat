...
- **原件（Original）**：文件落盘的完整字节内容。原件一经写入永不原地修改（重命名只改元数据，重新生成必产生新文件），因此可按不可变内容对待。
- **签名内容链接（Signed Content Link）**：服务端签发的限时公开文件访问 URL，签名绑定用户与文件、到期失效。持链接者在有效期内可读对应内容，无需登录态；用于浏览器与 CDN 可缓存的媒体直连加载。
- **缩略图变体（Thumbnail Variant）**：由图片类原件派生的低分辨率副本，分两档——列表/气泡档（≤400px）与预览档（≤1280px）；变体一经生成同样不可变，未生成的变体可按需补生成。
