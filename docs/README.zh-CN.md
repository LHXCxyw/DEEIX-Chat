...
本仓库是基于 [DEEIX Chat](https://github.com/DEEIX-AI/DEEIX-Chat) 的个人 Fork 和二次开发项目，由个人独立维护，不代表 DEEIX Chat 官方发行版本。当前二次开发主要包含用户自有上游渠道与用户模型路由（BYOK），以及相关的后端、前端、配置和数据库迁移改动。具体修改记录请以本仓库提交历史为准。
