...
本仓库是基于 [DEEIX Chat](https://github.com/DEEIX-AI/DEEIX-Chat) 的个人 Fork 和二次开发项目，由个人独立维护，不代表 DEEIX Chat 官方发行版本。当前二次开发主要包含用户自有上游渠道与用户模型路由（BYOK）、带 IDE 式资源管理器的 AI 项目工作区，以及相关的后端、前端、配置和数据库迁移改动。具体修改记录请以本仓库提交历史为准。

### 二改核心能力

- **用户自有上游渠道**：支持新增、编辑、启用、停用和删除个人模型渠道，不改变平台公共上游配置。
- **用户模型管理**：可在个人渠道下配置模型名称、协议、显示名称和模型能力信息。
- **用户模型路由**：个人模型与平台模型并列展示，选择个人模型后请求通过用户自己的上游渠道发送。
